...
df1 = spark.read.format("csv").option("header", "true").option("multiline", "true").load("/content/drive/MyDrive/smapledata.csv")
df2=df1.select("id",
               ...
               f.when(f.col('ind')==f.lit('AD'),f.concat_ws(",",f.col("fname"),f.col("lname"),f.col("apartment"),f.col("street"))).otherwise("null").alias("address"),
...
.withColumn("phone",f.split(f.col("address"),',').getItem(3))
